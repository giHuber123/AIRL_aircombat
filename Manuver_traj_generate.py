...
class ExpertRecorder:
    ...
    def save(self, filename="Manuver_traj.pkl"):
...
def test_maneuver():
    ...
    reward_list = []
    raw_obs = agent0.get_raw_observation(env, env.task)
...
def main():
    ...
    while True:
        ...
        action0 = agent0.get_action(env, env.task)
        action1, _ = agent1.get_action(env, env.task)

        # 3. 执行动作，获取环境反馈
        next_obs, reward, done, info = env.step([action0, action1])
        dones = np.array(done).all()
        recorder.record_step(raw_obs, dones)
        raw_obs = agent0.get_raw_observation(env, env.task)
        # 4. 更新状态
        obs = next_obs
    ...
    recorder.save("Manuver_traj.pkl")
    env.close()
