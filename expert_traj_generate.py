...
class PursueAgent(BaselineAgent):
    def __init__(self, agent_id) -> None:
        ...
        self.self_state_var = [
            c.attitude_roll_rad,  # 0. roll      (unit: rad)
            c.attitude_pitch_rad,  # 1. pitch     (unit: rad)
            c.velocities_u_mps,  # 2. v_body_x   (unit: m/s)
            c.velocities_v_mps,  # 3. v_body_y   (unit: m/s)
            c.velocities_w_mps,  # 4. v_body_z   (unit: m/s)
            c.velocities_vc_mps,  # 5. vc        (unit: m/s)
            c.position_h_sl_m  # 6. altitude  (unit: m)
        ]
    ...
    def get_raw_observation(self, env, task):
        ego_uid, enm_uid = list(env.agents.keys())[self.agent_id], list(env.agents.keys())[(self.agent_id + 1) % 2]
        raw_obs = env.agents[ego_uid].get_property_values(self.self_state_var)

        delta_values = self.set_delta_value(env, task)
        ...
        obs = np.zeros(12)
        obs[0] = delta_values[0]
        obs[1] = delta_values[1]
        obs[2] = delta_values[2]
        obs[3] = altitude
        obs[4] = np.sin(roll)
        obs[5] = np.cos(roll)
        obs[6] = np.sin(pitch)
...
class ManeuverAgent(BaselineAgent):
    def __init__(self, agent_id, maneuver: Literal['l', 'r', 'n']) -> None:
        ...
        self.self_state_var = [
            c.position_h_sl_ft,  # 0 altitude
            c.attitude_roll_rad,  # 1 roll      (unit: rad)
            c.attitude_pitch_rad,  # 2 pitch     (unit: rad)
            c.velocities_vt_fps,  # 3 v
            c.velocities_q_rad_sec,  # 4 pitch rate
            c.velocities_p_rad_sec,  # 5 roll rate
            c.aero_alpha_deg,  # 6 alpha
            c.aero_beta_rad,  # 7 beta
            c.accelerations_n_pilot_x_norm,  # 8 G_x
            c.accelerations_n_pilot_y_norm,  # 9
            c.accelerations_n_pilot_z_norm,  # 10
            c.fcs_left_aileron_pos_norm,  # 11
            c.fcs_right_aileron_pos_norm,  # 12
            c.fcs_elevator_pos_norm,  # 13
            c.fcs_rudder_pos_norm  # 14
        ]
...
class ExpertRecorder:
    def __init__(self):
        self.all_trajectories = []  # 存储所有局的数据
        self.current_episode = []  # 存储当前正在飞的这一局

    def record_step(self, obs, action, next_obs, done):
        """
        在每个 env.step() 之后调用
        obs: 14维向量
        action: 4维向量
        done: 布尔值 (terminated or truncated)
        """

        if action is None:
            action = np.zeros(4)  # 假设动作维度是4

        # 必须确保存入的是 copy，防止引用干扰
        step_data = {
            'obs': obs.copy() if isinstance(obs, np.ndarray) else obs,
            'action': action.copy() if isinstance(action, np.ndarray) else action,
            'next_obs': next_obs.copy() if isinstance(next_obs, np.ndarray) else next_obs,
            'done': done
        }
        ...
        if done:
            # 这一局结束，将整局存入大列表
            self.all_trajectories.extend(self.current_episode)
            self.current_episode = []  # 重置，准备下一局

    def save(self, filename="experts_data.pkl"):
        """将所有积累的局保存到单个 pkl 文件中"""
        with open(filename, 'wb') as f:
            pickle.dump(self.all_trajectories, f)
        print(f"成功保存 {len(self.all_trajectories)} 条数据点到 {filename}")
...
def main(episode):
    ...
    for i in range(episode):
        ...
        while True:
            # 1. 在当前状态 obs 下，决定动作 action
            action0 = agent0.get_action(env, env.task)
            action1 = agent1.get_action(env, env.task)

            raw_obs = agent1.get_raw_observation(env, env.task)
            norm_action = np.zeros(4)
            norm_action[0] = action0[0] * 1.0 / 20 - 1
            norm_action[1] = action0[1] * 1.0 / 20 - 1
            norm_action[2] = action0[2] * 1.0 / 20 - 1
            norm_action[3] = action0[3] * 1.0 / 58 + 0.4
            # 2. 【关键】在执行 step 之前记录！
            # 这样记录的就是：在 step_0 时，面对 obs_0，决定做 action_0
            # 此时 done 还是上一步的状态（初始为 False）

            # 3. 执行动作，获取环境反馈
            next_obs, reward, done, info = env.step([action0, action1])
            next_raw_obs = agent1.get_raw_observation(env, env.task)
            dones = np.array(done).all()
            recorder.record_step(raw_obs, norm_action, next_raw_obs, dones)
            # 4. 更新状态
            obs = next_obs

            # 5. 判断是否结束
            if np.array(done).all():
                # 【可选】如果你需要记录最后一步的终止状态（s_terminal）
                # 可以在这里额外调用一次 recorder，并标记 done=True
                print(info)
                break

    recorder.save("expert_traj_test.pkl")
    env.close()


def print_memory_usage(label="当前"):
    # 获取当前进程 ID
    process = psutil.Process(os.getpid())
    # 获取内存信息 (单位是字节，除以 1024**2 转换为 MB)
    mem_mb = process.memory_info().rss / (1024 ** 2)
    print(f"[{label}] 内存占用: {mem_mb:.2f} MB")
